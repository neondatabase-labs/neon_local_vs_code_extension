...
export class DockerService {
    ...
    private async pullImage(): Promise<void> {
        try {
            await this.docker.getImage('neondatabase/neon_local:v1.1').inspect();
        } catch {
            const authConfig = this.getDockerAuthConfig();
            const pullOpts = authConfig ? { authconfig: authConfig } : {};

            await new Promise((resolve, reject) => {
                this.docker.pull('neondatabase/neon_local:v1.1', pullOpts, (err: any, stream: any) => {
                    if (err) {
                        reject(err);
                        return;
                    }
    ...
    private async startProxy(options: {
        ...
        if (persistentApiToken) {
            ...
            const containerConfig: any = {
                Image: 'neondatabase/neon_local:v1.1',
